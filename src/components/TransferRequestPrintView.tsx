...
const TransferRequestPrintView = ({ request, accounts, companyName = 'شركة الرمال الناعمة', scale = 1 }: Props) => {
  ...
  // ضع كل البنود في صفحة واحدة دائماً
  const items = request.items;
  const pages: TransferRequestItem[][] = [items];
  const totalPages = 1;

  // ارتفاع الصف الافتراضي 23مم، يقل تلقائياً إذا كانت البنود كثيرة جداً لتسع صفحة واحدة
  // المساحة المتاحة للجدول تقريباً: 297 - هوامش - هيدر - إجمالي - ملاحظات - توقيعات ≈ 175مم
  const AVAILABLE_TABLE_HEIGHT = 175; // mm
  const headerHeight = 30; // mm
  const MAX_ROW_HEIGHT = 23; // mm
  const MIN_ROW_HEIGHT = 8; // mm
  const itemsCount = Math.max(items.length, 1);
  const computedRowHeight = Math.min(
    MAX_ROW_HEIGHT,
    Math.max(MIN_ROW_HEIGHT, (AVAILABLE_TABLE_HEIGHT - headerHeight) / itemsCount)
  );
