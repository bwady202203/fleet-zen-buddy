...
const PurchaseReturn = () => {
  ...
  const [formData, setFormData] = useState({
    invoiceNumber: getNextInvoiceNumber('purchase-return'),
  ...
  const purchaseReturns = getInvoicesByType('purchase-return');
  const supplierAccounts = accountSearch ? searchAccounts(accountSearch).filter(acc => acc.level === 4) : [];
  ...
  const addItem = () => {
    if (!currentItem.itemName || !currentItem.accountId) {
      toast({ title: "خطأ", description: "يرجى إدخال اسم الصنف والحساب", variant: "destructive" });
      return;
    }
    const total = currentItem.quantity * currentItem.unitPrice;
    const taxAmount = total * (currentItem.taxRate / 100);
    const newItem: InvoiceItem = { id: `item-${Date.now()}`, ...currentItem, total, taxAmount };
    setFormData({ ...formData, items: [...formData.items, newItem] });
    setCurrentItem({ itemName: "", quantity: 1, unitPrice: 0, taxRate: 15, accountId: "" });
  ...
  const handleSubmit = () => {
    if (!formData.customerSupplierName || !formData.customerSupplierAccount || formData.items.length === 0) {
      toast({ title: "خطأ", description: "يرجى إدخال بيانات المورد وإضافة صنف واحد على الأقل", variant: "destructive" });
      return;
    }
    const { subtotal, taxTotal, total } = calculateTotals();
    addInvoice({ ...formData, type: 'purchase-return', subtotal, taxTotal, total });
    toast({ title: "تم الحفظ بنجاح", description: `تم حفظ مرتجع المشتريات رقم ${formData.invoiceNumber}` });
    setDialogOpen(false);
    resetForm();
  };
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link to="/accounting"><ArrowRight className="h-6 w-6" /></Link>
              <div>
                <h1 className="text-3xl font-bold">مرتجعات المشتريات</h1>
                <p className="text-muted-foreground mt-1">إدارة مرتجعات المشتريات</p>
              </div>
            </div>
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button size="lg"><Plus className="ml-2" />مرتجع جديد</Button>
              </DialogTrigger>
              <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                <DialogHeader><DialogTitle>مرتجع مشتريات جديد</DialogTitle></DialogHeader>
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {accountSearch && supplierAccounts.length > 0 && (
      ...
      <main className="container mx-auto px-4 py-8">
        <Card>
          <CardHeader><CardTitle>قائمة مرتجعات المشتريات</CardTitle></CardHeader>
          <CardContent>
