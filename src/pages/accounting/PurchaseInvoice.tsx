...
const PurchaseInvoice = () => {
  ...
  const [formData, setFormData] = useState({
    invoiceNumber: getNextInvoiceNumber('purchase'),
  ...
  const purchaseInvoices = getInvoicesByType('purchase');
  ...
  const handleSubmit = () => {
    if (!formData.customerSupplierName || !formData.customerSupplierAccount || formData.items.length === 0) {
      toast({
        title: "خطأ",
        description: "يرجى إدخال بيانات المورد وإضافة صنف واحد على الأقل",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      ...
      description: `تم حفظ فاتورة المشتريات رقم ${formData.invoiceNumber}`,
    });
              ...
              <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleSubmit} className="flex-1">حفظ الفاتورة</Button>
                ...
                <TableBody>
                  {purchaseInvoices.map(invoice => (
                    <TableRow key={invoice.id}>
                      ...
                      <TableCell>{invoice.total.toFixed(2)} ريال</TableCell>
                      <TableCell>
